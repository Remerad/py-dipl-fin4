...
urlpatterns = [
    ...
    path('user/register/confirm', ConfirmAccount.as_view(), name='user-register-confirm'),  #работает
    ...
    path('user/password_reset', reset_password_request_token, name='password-reset'),   #Шевелится, кодировка сообщения?
    ...
    path('categories', CategoryView.as_view(), name='categories'),  #Работает
    path('products', ProductInfoView.as_view(), name='shops'),  #Работает
    path('basket', BasketView.as_view(), name='basket'),  #Работает
    path('order', OrderView.as_view(), name='order'),  #Работает
]